...
A tool's commands — built via the public ``ToolFactory`` API — must never pin a
weak-referenced owning agent. After the only strong reference to the observer is
dropped and ``gc.collect()`` runs, the agent is reclaimed even while the command
mapping and its closures are still held. This proves FR6: no tool, its closures,
or the agent's command surface keeps a stopped ``BaseAgent`` alive.
...
def _build_registry(observer: Mock) -> object:
    """Wire TeamTool + PlanningTool through the public ToolFactory and build the commands.
    ...
    This branch predates the command-registry surface, so the equivalent public
    entry point is ``get_commands()``: the same per-card command closures, keyed
    by param model instead of by name. The retention edge under test is identical.
    """
    factory = ToolFactory([TeamTool(), PlanningTool(vector_store=False)], observer=observer)
    return factory.get_commands()
