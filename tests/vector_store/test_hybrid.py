...
def _proxy(hits: list[SearchHit]) -> MagicMock:
    ...
    It carries no ``embed``: after story 49-3 the store embeds nothing and the
    query leg goes through the caller's own :func:`_embedder`.
    """
    proxy = MagicMock()
...
def _embedder() -> MagicMock:
    """Build the caller's own embedding service, returning one fixed vector."""
    embedder = MagicMock()
...
class TestHybridSearch:
    ...
    def test_orders_best_first(self) -> None:
        ...
        result = hybrid_search(["b"], proxy, _embedder(), "c", "query", top_k=5)
    ...
    def test_returns_the_raw_vector_scores_for_labelling(self) -> None:
        ...
        assert hybrid_search([], proxy, _embedder(), "c", "query", top_k=5).vector_scores == {
    ...
    def test_threshold_applies_to_the_raw_score_before_normalisation(self) -> None:
        ...
        result = hybrid_search(
            [], proxy, _embedder(), "c", "query", top_k=5, score_threshold=0.5
        )
    ...
    def test_over_fetches_past_top_k(self) -> None:
        ...
        proxy = _proxy([])
        hybrid_search([], proxy, _embedder(), "c", "query", top_k=5)
    ...
    def test_result_is_not_cut_to_top_k(self) -> None:
        ...
        assert len(hybrid_search([], proxy, _embedder(), "c", "query", top_k=2).ranked) == 6
    ...
    def test_no_vector_hits_degrades_to_keyword_order(self) -> None:
        ...
        result = hybrid_search(["a", "b", "c"], None, _embedder(), "c", "query", top_k=5)
    ...
    def test_alpha_is_forwarded_to_the_fusion(self) -> None:
        proxy = _proxy([SearchHit(ref_type="t", ref_id="a", text="", score=0.9)])
        result = hybrid_search(["b"], proxy, _embedder(), "c", "query", top_k=5, alpha=0.1)
        assert [key for key, _ in result.ranked] == ["b", "a"]

    def test_a_missing_embedder_degrades_to_the_keyword_leg(self) -> None:
        """AC 19: ``embedder`` is forwarded, and a missing one skips the leg."""
        proxy = _proxy([SearchHit(ref_type="t", ref_id="a", text="", score=0.9)])
        result = hybrid_search(["b"], proxy, None, "c", "query", top_k=5)
        assert [key for key, _ in result.ranked] == ["b"]
        assert result.vector_scores == {}
...
class TestSemanticScores:
    ...
    def test_maps_hits_to_ref_id_scores(self) -> None:
        ...
        assert semantic_scores(proxy, _embedder(), "c", "query", 5) == {"a": 0.9, "b": 0.4}
    ...
    def test_returns_raw_scores_not_normalised_ones(self) -> None:
        ...
        assert semantic_scores(proxy, _embedder(), "c", "query", 5) == {
            "a": pytest.approx(0.42)
        }
    ...
    def test_preserves_backend_ranking_order(self) -> None:
        ...
        assert list(semantic_scores(proxy, _embedder(), "c", "query", 5)) == ["a", "b"]

    def test_forwards_collection_and_top_k_to_the_backend(self) -> None:
        proxy = _proxy([])
        semantic_scores(proxy, _embedder(), "planning", "query", 7)
        proxy.search.assert_called_once_with("planning", [0.1, 0.2, 0.3], 7)

    def test_the_query_is_embedded_through_the_callers_own_service(self) -> None:
        """AC 19: the store is never asked to embed anything."""
        proxy = _proxy([])
        embedder = _embedder()
        semantic_scores(proxy, embedder, "c", "query", 5)
        embedder.embed.assert_called_once_with(["query"])
        proxy.embed.assert_not_called()

    def test_no_proxy_yields_no_hits(self) -> None:
        assert semantic_scores(None, _embedder(), "c", "query", 5) == {}

    def test_no_embedder_yields_no_hits(self) -> None:
        proxy = _proxy([SearchHit(ref_type="entity", ref_id="a", text="", score=0.9)])
        assert semantic_scores(proxy, None, "c", "query", 5) == {}
        proxy.search.assert_not_called()
    ...
    def test_empty_embedding_yields_no_hits(self) -> None:
        ...
        assert semantic_scores(proxy, embedder, "c", "query", 5) == {}
        proxy.search.assert_not_called()
    ...
    def test_an_empty_embedding_is_logged(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.WARNING):
            semantic_scores(proxy, embedder, "planning", "query", 5)
    ...
    def test_a_missing_embedder_is_logged(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.WARNING):
            semantic_scores(_proxy([]), None, "c", "query", 5)
        assert "No embedder" in caplog.text
...
class TestSemanticOptOut:
    ...
    def test_the_backend_is_not_queried(self) -> None:
        proxy = _proxy([SearchHit(ref_type="t", ref_id="a", text="", score=0.9)])
        embedder = _embedder()
        hybrid_search(["b"], proxy, embedder, "c", "query", top_k=5, semantic=False)
    ...
    def test_it_does_not_warn_about_a_missing_proxy(
        ...
        proxy = _proxy([])
        with caplog.at_level(logging.WARNING):
            hybrid_search(["b"], proxy, _embedder(), "c", "query", top_k=5, semantic=False)
        assert "No vector store proxy" not in caplog.text

    def test_the_keyword_leg_still_ranks(self) -> None:
        proxy = _proxy([SearchHit(ref_type="t", ref_id="a", text="", score=0.9)])
        result = hybrid_search(
            ["b", "c"], proxy, _embedder(), "col", "query", top_k=5, semantic=False
        )
