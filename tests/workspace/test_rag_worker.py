...
class TestWhatTheWorkerIs:
    ...
    def test_nothing_this_module_exports_shadows_a_builtin(self) -> None:
        ...
        ``rag/actor.py``. A later ``except IndexError:`` there would raise
        ``TypeError`` at runtime instead of catching, and mypy would not
        complain. Ruff's ``A`` rules are not selected in this package, so this
        spec is the gate.
        """
        import builtins
