...
def _card(**params: Any) -> SkillTool:
    ...
    return SkillTool(skills=Skills(**params))
...
class TestCardSurface:
    def test_the_card_declares_exactly_one_field(self) -> None:
        assert set(SkillTool.model_fields) == {"skills"}

    def test_the_capability_ships_on(self) -> None:
        assert SkillTool.model_fields["skills"].default is True
        assert SkillTool().skills is True
...
class TestMenu:
    ...
    def test_an_empty_library_contributes_the_empty_string(self) -> None:
        card = SkillTool(skills=Skills(skills=[]))
    ...
    def test_the_default_card_renders_an_empty_menu(self) -> None:
        """``skills=True`` means "enabled with defaults", and the default library is empty."""
...
class TestSerialization:
    def test_it_round_trips_through_a_dict(self) -> None:
        ...
        assert isinstance(restored.skills, Skills)
        assert restored.skills.skills == [REFUND, ESCALATION]
        assert restored.skills.expose == {SYSTEM_PROMPT, TOOL_CALL}
    ...
    def test_it_round_trips_through_json_preserving_every_entry_in_order(self) -> None:
        ...
        assert isinstance(restored.skills, Skills)
        assert [
            (entry.name, entry.description, entry.content) for entry in restored.skills.skills
        ] == [
            (REFUND.name, REFUND.description, REFUND.content),
            (ESCALATION.name, ESCALATION.description, ESCALATION.content),
        ]
        assert restored.skills.expose == {SYSTEM_PROMPT, TOOL_CALL, COMMAND}

    def test_a_disabled_card_round_trips_as_false(self) -> None:
        card = SkillTool(skills=False)
        assert SkillTool.model_validate_json(card.model_dump_json()).skills is False
