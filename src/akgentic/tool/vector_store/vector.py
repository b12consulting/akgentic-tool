"""Vector primitives — the layer every backend, actor and protocol here is built on.

This file lives inside ``vector_store/`` because the two halves are one subsystem, not
two: the primitives below and the service layer beside them are consumed together and
shipped by the same ``[vector_search]`` extra. One package, one place.
