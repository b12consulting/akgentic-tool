...
arriving under that hold is refused immediately, naming the holder; reads are
untouched and keep working throughout.

**Two actors and one running command.** The command is sent to
``#SandboxActor-<workspace>`` as a tell and the sandbox tells the report back, so
the blocking subprocess runs on the sandbox's own thread — the thread it was
always going to run on — and this one goes on draining its mailbox. That is what
keeps reads working during a run and makes a refused mutation cost one turn
instead of the run's whole duration. This actor sends one command at a time,
because only it knows when the tree has been committed and a second run started
early would sweep the first run's files into its own discovery.

The deferred-result mechanism (ADR-033) supplies the **cache half** — ``get`` /
``deliver`` / ``fail``, the LRU, the negative TTL and the in-flight set. Its
worker half is not used here: nothing is spawned, and ``request()`` is never
called. Everything the ask path does is bounded — one file read, one write, a few
short-lived ``git`` forks under an explicit timeout — and never external.
"""
...
"""Opening words of every refusal the running command causes.

Fixed wording because it is what an agent recognises across all six refused
operations — the mutations. A second ``workspace_exec`` is not one of them: it
queues rather than refusing, which is the asymmetry story 47-1 introduced.
"""
...
class WorkspaceActor(DeferredResultActor[WorkspaceConfig, WorkspaceState, str, ExecOutcome]):
    """Team singleton owning one workspace tree, the observations, the gate, and the run.
    ...
    keyed by run id — its **cache half** only. The base supplies ``_slots`` and
    ``_in_flight``; do not shadow them and do not add a second cache. Its LRU and
    its negative TTL are two of the seven deferred rules, and reimplementing
    either is how a partial adoption starts. Its worker half is unused: nothing
    is spawned, ``request()`` is never called, and :meth:`worker_class` raises.

    ``_in_flight`` is maintained by :meth:`_start_run`, which is what ``request``
    used to do for it. :meth:`exec_status` **does** read it, and that is
    deliberate: a run is running iff it is in flight, and the earlier prohibition
    forced the question onto a map with a different capacity, which answered
    ``RUNNING`` for settled runs. Reading an own attribute on an own thread is
    not a partial adoption; duplicating the cache would be.
    """
    ...
    ##
    ## Exec — the queue, the running command, and the discovered commit
    ##
    ...
    def _journalled(
        ...
        Order per mutation: refuse if a run holds the tree → commit anything nobody
